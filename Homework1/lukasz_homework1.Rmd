...
PC = data.table(svd.out$v,pData(bottomly.eset))
ggplot(PC) + geom_point(aes(x=V2, y=V3, col=as.factor(strain))) + ggtitle('PCs separating strains (V2 vs V3)')
ggsave("lukasz_problem2.pdf")
...
plot(svd.out$u[,1], svd.out$u[,2],pch=20,main='Top 2 left singular vectors')
dev.off()
```
...
PC.left = data.table(svd.out$u)
PC.left = gather(PC.left[,1:5])
ggplot(PC.left) + geom_violin(aes(x=as.factor(key), y=value),draw_quantiles = c(0.25, 0.5, 0.75)) + xlab('Singular vector') + ggtitle('Top 5 left singular vectors')
ggsave("lukasz_problem4.pdf")
...
```{r}
tsne_out <- Rtsne(edata,pca=FALSE)
tsne_out = data.table(tsne_out$Y)
k.data <- kmeans(edata, centers=5)
tsne_out = data.table(tsne_out, cluster = as.factor(k.data$cluster))
ggplot(tsne_out) + geom_point(aes(x=V1, y=V2, col=cluster)) + ggtitle('t-SNE for clustered genes')
ggsave("lukasz_problem5.pdf")
